  ...
  return (
    <div class="flex flex-col justify-center items-center h-screen bg-cover bg-no-repeat" style="background-image: url('/background.svg')">
      <form onSubmit={handleSubmit} class="flex flex-col items-center p-10 w-5/6 sm:w-96 bg-light-purple rounded-xl shadow-md gap-5">
        ...
        <input type="text" name="username" value={username()} onInput={(e) => setUsername(e.target.value)} class="w-full h-10 sm:h-12 px-4 border-2 border-purple rounded-lg shadow-md" placeholder="Логин" autocomplete="username" required/>
        
        <input type="password" name="password" value={password()} onInput={(e) => setPassword(e.target.value)} class="w-full h-10 sm:h-12 px-4 border-2 border-purple rounded-lg shadow-md" placeholder="Пароль" autocomplete="current-password" required/>
        ...
        {error() && <div class="text-red text-center">{error()}</div>}  
